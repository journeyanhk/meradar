...
const state = new Map(); // token(lower) -> { buyers:Set, buyTs:number[], lastPriceWei:bigint, volBnb:number, funds:bigint, offers:bigint, peakMcapUsd:number }
const WINDOW_MS = 30 * 60_000;

function get(token) {
  const k = token.toLowerCase();
  let s = state.get(k);
  if (!s) { s = { buyers: new Set(), buyTs: [], lastPriceWei: 0n, volBnb: 0, funds: 0n, offers: 0n, peakMcapUsd: 0 }; state.set(k, s); }
  return s;
}
...
export function onTrade({ token, account, price, cost, funds, offers, isBuy, ts = Date.now() }) {
  ...
  if (offers != null) s.offers = offers;
...
export function curveMetrics(token, supplyHuman, bnbUsd) {
  ...
  const offersHuman = Number(s.offers) / 1e18;
